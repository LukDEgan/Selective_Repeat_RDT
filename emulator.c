...
void init(void) /* initialize the simulator */
{
  ...
  if (lossprob != 0.0 || corruptprob != 0.0) {
    printf(
        "If you want loss or corruption to only occur in one direction, choose "
        "the direction: 0 A->B, 1 A<-B, 2 A<->B (both directions) :");
...
/* A or B is sending to network  */
{
  ...
  /* simulate losses: */
  if (jimsrand() < lossprob && (!(AorB == B && corruptdirection == A) &&
...
terminate:
  printf(
      " Simulator terminated at time %f\n after attempting to send %d msgs "
      "from layer5\n",
      time, nsim);
  printf("number of messages dropped due to full window:  %d \n", window_full);
  printf(
      "number of valid (not corrupt or duplicate) acknowledgements received at "
      "A:  %d \n",
      new_ACKs);
  printf(
      "(note: a single acknowledgement may have acknowledged more than one "
      "packet - if cumulative acknowledgements are used)\n");
